...
export async function run(bot: Bot, interaction: CommandInteraction) {
    ...
    let code = suffix?.replace(
        /^\u0060\u0060\u0060(js|javascript ?\n)?|\u0060\u0060\u0060$/g,
        ''
    )
    let inspect = (e: string, colors?: any) =>
        typeof e === 'string' ? e : util.inspect(e, { depth: 0, colors })
    ...
    try {
        ...
        let type = result
            ? `[${result.constructor ? result.constructor.name : 'Object'}] => `
            : ''
