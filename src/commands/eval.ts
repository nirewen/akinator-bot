...
export async function run(bot: Bot, interaction: CommandInteraction) {
    ...
    try {
        ...
        interaction.reply({ content: Formatters.codeBlock('js', message), ephemeral: true })
    } catch (error) {
        console.error(error)
        interaction.reply({ content: Formatters.codeBlock('js', error), ephemeral: true }).catch(err => console.log(err.message))
    }
}